...
llm_openai = ChatOpenAI(model="gpt-5.4-mini").bind_tools(tools)
...
message = [
    SystemMessage(content="You are a helpful assistant that can calculate the shipping cost for a given weight and destination"),
    HumanMessage(content="What is the shipping cost for a 10kg package to USA in express mode?")
]
...
"""
common practices for tools usage:
1. link only the tools that are relevant to the agent's task/goal
2. if you have more than 10 tools, first decide the type of query and then use the appropriate tools
3.ensure that instructions are provided to the LLM to use the tools correctly
"""